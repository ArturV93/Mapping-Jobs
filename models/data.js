  ...
  this.jobs = function(req,res) {
    connection.acquire(function(err,con){
      ...
      if(req.body.occupation !== undefined && req.body.occupation !== ''){
        var occupation = "SELECT * FROM jobs where occupation='" + req.body.occupation + "' AND DATE(datetime) = DATE(NOW()) ORDER BY num_jobs";
        ...
        var occupation = "SELECT county, sum(num_jobs) as num_jobs, AVG(salary) as salary FROM jobs where DATE(datetime) = DATE(NOW()- INTERVAL 35 day) GROUP BY county ORDER BY num_jobs";
